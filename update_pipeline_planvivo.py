...
CHROMA_BASE_DIR = "app/embeddings_plan_vivo"
SUBFOLDERS = ["Standard_documents", "Project_documents"]
TOKEN_LIMIT = 280_000
MAX_TOKENS_PER_CHUNK = 8191
BATCH_SIZE = 50
SEMANTIC_BATCH_SIZE = 200   # batch size for semantic splitting
...
def load_csv(path):
    ...
    except Exception as e: print(f"❌ CSV load failed: {path} | {e}")
    return docs
...
def load_kml(path):
    ...
    except Exception as e: print(f"❌ KML load failed: {path} | {e}")
    return docs
                    ...
                    with zf.open(name) as kml_file:
                        ...
                        for feature in k.features():
                            for placemark in feature.features():
                                ...
                                text = f"Name: {name}\nDescription: {desc}\nGeometry: {coords}"
                                docs.append(Document(page_content=text, metadata={"page": 1, "clause": "N/A"}))
    except Exception as e: print(f"❌ KMZ load failed: {path} | {e}")
    return docs

def load_file(path):
    lower = path.lower()
    if lower.endswith(".pdf"): return load_pdf(path)
    if lower.endswith((".docx", ".doc")): return load_docx(path)
...
def create_chroma_index():
    ...
    for subfolder in SUBFOLDERS:
        print(f"\n🔍 Processing PLAN_VIVO subfolder: {subfolder}")
