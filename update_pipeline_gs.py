# update_pipeline_gs_recursive.py
...
# === Loaders ===
def load_pdf(path): 
    try: return PyPDFLoader(path).load()
    except Exception as e: print(f"❌ PDF load failed: {path} | {e}"); return []
...
def load_excel(path):
    ...
    try:
        ...
        for sheet in xls.sheet_names:
            ...
            for i in range(0, len(text), 4000):  # bigger grouping
                docs.append(Document(page_content=text[i:i+4000], metadata={"page": 1, "clause": "N/A", "sheet": sheet}))
...
def create_chroma_index():
    embeddings = OpenAIEmbeddings(openai_api_key=os.getenv("OPENAI_API_KEY"), chunk_size=200)
    recursive_splitter = RecursiveCharacterTextSplitter(chunk_size=2000, chunk_overlap=200)  # ✅ only recursive
    ...
    for subfolder in SUBFOLDERS:
        ...
        for i in tqdm(range(0, len(chunks), BATCH_SIZE), desc=f"🔗 Embedding {subfolder}"):
            ...
            valid_docs = [
                Document(page_content=clean_text(d.page_content, d.metadata.get("source", "unknown")),
    ...
    print("✅ GS Recursive Pipeline completed successfully!")
